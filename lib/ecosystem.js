...
module.exports = {
  ...
  npmInstallTrailpacks (repos) {
    return repos.map(repo => {
      ...
      const npmi = spawnSync('npm', [ 'install' ], {
        shell: true,
        cwd: wd,
        stdio: [
          'ignore',
      ...
      console.log(npmi.output.toString())

      console.log('>> npm link trails')
      const npmlink = spawnSync('npm', [ 'link', 'trails' ], {
        cwd: wd,
        stdio: [
          'ignore',
          process.stderr,
      ...
      console.log(npmlink.output.toString())
  ...
  checkNodeSecurity (repos) {
    spawnSync('npm', [ 'install', '-g', 'nsp' ], { shell: true })
